...
export default function EggModule({ D, card, inp, textPrimary, textSecondary, textMuted, bgCard2, borderColor, borderColor2, currentUser }) {
  ...
  const typeColors = { entrada: "#10b981", venta: "#d71920", merma: "#e03131", rotos: "#f03e3e", trizados: "#d6336c", ajuste_entrada: "#8b5cf6", ajuste_salida: "#f59e0b" };
  ...
  const daySummary = [
    ...
    { label: "Entradas", count: dayEntradas.length, detail: `${dayEntradas.reduce((s, m) => s + Number(m.huevos || 0), 0) / EGG_BOX_UNITS} cajas`, color: "#d71920" },
  ...
  return <div>
    ...
    <div style={{ ...card, padding:8, display:"flex", gap:6, marginBottom:18, overflowX:"auto" }}>
      {tabs.map(t => <button key={t.id} onClick={() => setTab(t.id)} style={{ padding:"9px 14px", borderRadius:9, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontWeight:700, fontSize:12, fontFamily:"inherit", background:tab===t.id?"linear-gradient(135deg,#d71920,#ef2b32)":"transparent", color:tab===t.id?"#fff":textSecondary }}>{t.label}</button>)}
    </div>

    {tab === "dashboard" && <>
      <div className="dashboard-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:16 }}>
        {[
          ["Cajas completas", totalBreakdown.cajas, "📦", "#d71920"],
      ...
      <div className="dashboard-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:18 }}>
        {[
          ...
          ["Ganancia huevos", fmt(profit), "Sin mezclar otros productos", "#d71920"],
      ...
      <div style={card}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          ...
          <button onClick={() => { setTab("movimientos"); setMovDate(todayLocalISO()); }} style={{ border:"none", background:"none", color:"#d71920", fontSize:12, fontWeight:700, cursor:"pointer" }}>Ver todos →</button>
        </div>
        {(() => {
                  ...
                  {isExpanded && <div onClick={e=>e.stopPropagation()} style={{ marginTop:16, paddingTop:14, borderTop:`1px solid ${borderColor}` }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(7,minmax(62px,1fr))", gap:6, overflowX:"auto", paddingBottom:4 }}>
                      {weekDays.map(day => {
                        ...
                        return <button key={day} onClick={()=>setExpandedWeekDay(day)} style={{ minWidth:62, padding:"8px 4px", borderRadius:9, border:`1px solid ${active?"#d71920":borderColor2}`, background:active?"linear-gradient(135deg,#d71920,#ef2b32)":bgCard2, color:active?"#fff":textSecondary, cursor:"pointer", fontFamily:"inherit" }}>
                    ...
                    <div style={{ marginTop:12, padding:12, borderRadius:10, background:bgCard2 }}>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(95px,1fr))", gap:6, marginBottom:10 }}>
                        {[
                          ["Entradas",selectedSummary.entradas,"#d71920"],
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1,display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:8}}>
            {movWindowDays.map(d => { const { dayMonth, weekday } = formatDayLabel(d); const selected = d === movDate; return (
              <button key={d} onClick={()=>setMovDate(d)} style={{padding:"9px 4px",borderRadius:10,border:"none",cursor:"pointer",textAlign:"center",background:selected?"linear-gradient(135deg,#d71920,#ef2b32)":bgCard2,color:selected?"#fff":textSecondary}}>
    ...
    {tab === "balance" && <>
      <div style={{ ...card, marginBottom:16, display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div style={{ flex:1 }} />
        <div style={{ padding:"8px 18px", borderRadius:10, background:"linear-gradient(135deg,#d71920,#ef2b32)", color:"#fff", fontWeight:800, fontSize:14 }}>{formatDayLabel(balanceDate).dayMonth}</div>
